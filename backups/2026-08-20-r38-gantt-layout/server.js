...
function createFromTemplate(tpl, name, opts) {
  ...
  return {
    id: uid(), name: name || tpl.name, templateId: tpl.id, category: tpl.category,
    summary: tpl.summary, icon: tpl.icon, color: tpl.color,     startDate,
    type: opts.type || 'C端',
    level: opts.level || 'B',
  ...
  if (p.startsWith('/api/')) {
    ...
    if (p === '/api/projects/import' && req.method === 'POST') {
      ...
      try {
        ...
        if (body.kind === 'json') {
          ...
          np = { name: (body.filename || '导入项目').replace(/\.[^.]+$/, ''), phases: parsed.phases, tasks: parsed.tasks, summary: '', category: '', icon: '◆', color: '#0a84ff', startDate: null, startCell: parsed.startCell || null };
        }
    ...
    if (m) {
      ...
      if (m[0].endsWith('/reschedule') && req.method === 'POST') {
        proj.tasks.forEach(t => { delete t.startRule; delete t.dueRule; delete t.startF; delete t.dueF; }); // 重新排期 = 手动重排，彻底清除公式（含原文）
        scheduleTasks(proj.phases, proj.tasks, proj.startDate || isoDate(new Date()));
      ...
      if (!tid) {
        ...
        if (req.method === 'PUT') {
          ...
          if (body.startDate) { proj.startDate = body.startDate; needSchedule = true; }
          if (needSchedule) {
      ...
      if (req.method === 'PUT') {
        ...
        if (body.startDate) { t.startDate = body.startDate; delete t.startRule; delete t.startF; } // 手动改开始 → 覆盖该任务自己的公式
...
server.listen(PORT, () => console.log('Multi-project kanban running at http://localhost:' + PORT));
