...
function extractZip(zipPath, root) {
  ...
  const errs = [];
  // 方案 A：tar.exe（Windows 10 17063+ 内置 bsdtar，支持 zip）—— spawnSync 数组参数，路径零转义。
  ...
  let r = cp.spawnSync(tarExe, ['-xf', zipPath, '-C', root], { windowsHide: true, timeout: 600000 });
  if (r.status === 0 && !r.error) return 'tar';
  errs.push('tar(exit=' + r.status + (r.error ? ' ' + r.error.code : '') + '): ' + stderrTail(r.stderr));
  ...
  r = cp.spawnSync('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-EncodedCommand', enc], { windowsHide: true, timeout: 600000 });
  if (r.status === 0 && !r.error) return 'expand';
  errs.push('Expand-Archive(exit=' + r.status + (r.error ? ' ' + r.error.code : '') + '): ' + stderrTail(r.stderr));
