...
function buildTodoXlsx(projects, monIso, sunIso, kind) {
  ...
  const statusLabel = kind === 'today' ? '今日待办' : kind === 'month' ? '本月待办' : '本周待办';
  ...
  groups.forEach(({ p, list }) => list.forEach((t, ti) => {
    rows.push([ti === 0 ? safeCell(p.name) : '', safeCell(t.title), t.estimateDays || '', fmt(t.startDate), fmt(t.dueDate), t.overdue ? '⚠ 逾期' : statusLabel]);
  }));
  ...
  ws['!merges'] = merges;
  ws['!cols'] = [{ wch: 20 }, { wch: 36 }, { wch: 10 }, { wch: 12 }, { wch: 12 }, { wch: 12 }];
