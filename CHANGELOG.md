...
> 连续数分钟 502，用户每次只能放弃并改用手工覆盖。本机等价复现后定位为**同步子进程冻结事件循环**。

### 升级期间不再阻塞服务（核心修复）
- `lib/upgrade.js#extractZip` / `#backupDir`：`spawnSync` / `execSync` → **异步 `spawn`**。
  实测根因：解压全程事件循环被冻死，97 秒内端口 100% 无响应，前端轮询必然全超时 →
  只能显示「服务重启中…」。改异步后升级期间看板照常响应（复验端口不可达仅重启瞬间 1 秒）。
- 新增 `runChild()` 统一异步子进程执行器（带 stdout/stderr 捕获、行回调、硬超时）。
- 备份只读取不再改动根目录，顺带修掉 robocopy 默认 `/R:1000000 /W:30` 的重试陷阱（改为 `/R:2 /W:1`）。

### 真实进度上报
- `extractZip` 用 `tar -xvf` 逐条计数 + 读 zip EOCD 条目总数（`zipEntryCount`），
  通过 `onProgress` 把解压进度推到任务的 88%→95% 区间，前端能看到进度在动而不是干等。

### 升级包瘦身（22.5MB → 2.8MB）
- `tools/build_update_zip.py` **默认排除 `node_modules`**（`--with-deps` 可退回旧口径）。
  1139 文件 / 22.5MB → 92 文件 / 2.8MB，解压 97s → 9.1s；且不再覆盖运行中的依赖目录
  （旧口径下备份恰好排除 node_modules，依赖被覆盖坏则无法回滚）。
- 新增依赖自检 `missingDeps()`：包不带依赖，就在升级流程里核对 `package.json.dependencies`
  是否已装，缺则明确报错并提示 `npm install --omit=dev`，绝不留给重启后 500。

### 守护进程不再帮倒忙
- `watchdog.js#ensureServer`：端口无响应时先 `pidOfPort()` 查监听者——
  **进程存活 = 忙碌**，按宽限期跳过（无锁 4 轮≈60s、升级中 60 轮≈15min），
  不再对着同一端口连拉重复实例（生产实测原逻辑连拉 8 次，全部撞 EADDRINUSE 空转）；
  持续超宽限期才判定僵死并精确 `taskkill /F /PID` 后拉起（自愈能力不丢）。
- `watchdog.js#ensureTunnel`：新增两道闸门——**升级锁生效期间不重启隧道**；
  **本机源站也不响应时不重启隧道**（问题在源站，重启隧道只会带来 90s 冷却，
  曾把 10-30 秒的正常中断放大成 9 分钟公网不可达）。源站恢复后隧道守护自动恢复工作。
- 升级锁加**心跳**：`lib/upgrade.js` 每 30s 刷新锁内时间戳，watchdog 读到超过 3 分钟未刷新的锁
  即判为过期残留并忽略（此前"升级进程被杀 → 锁残留 → 热重载与自愈永久失效"）。
- 解锁改为**先写时间戳 0（过期标记）再尝试删除**：Windows 上删除可能被杀软/权限/安全策略拦截，
  删不掉也不会让锁继续生效。

### 前端提示更诚实
- 升级期间轮询失败不再伪造 98% 进度，改为「看板暂不可达（已等待 Ns）：正在解压覆盖文件或重启服务」；
- 超时提示补充可操作路径（1 分钟后刷新看页脚版本号 / 查 `logs/upgrade-*.log` 与 `upgrade-failed-*.zip`）；
- 确认弹窗去掉「公网约中断 10-30 秒」的乐观承诺，改为「页面可能短暂无响应，请勿关闭」。

### 回归
- `test/upgrade.integration.test.cjs`：extractZip 转 async 后同步更新，并新增
  解压进度回调 / `zipEntryCount` / `missingDeps` 只报真实缺失 / 锁心跳刷新与解锁 等断言。
- `test/watchdog-upgrade-lock.test.cjs`：场景 3 拆为「真崩→拉起」「忙碌→不重复拉起」「超宽限→强制重启」
  三例，新增「过期残留锁自动忽略」「新鲜锁仍生效」。
- `test/watchdog-tunnel.test.cjs`：新增「源站不响应→不重启隧道」「升级窗口→不重启隧道」
  「源站恢复后隧道守护恢复工作」三例。
- 全套 21 套件通过 + 前端 jsdom 冒烟 SMOKE_OK。
